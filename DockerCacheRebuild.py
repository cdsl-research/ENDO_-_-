...
#Dockerfile_Pass = "/home/c0a20028/SneakerBot-master"
fiile_name = Dockerfile_Pass.split('/')
fiile_name = fiile_name[-1]
tmp_list = []
...
try:
    with open(fiile_name+".mem", mode='r', newline='') as f:
...
with open(fiile_name+".csv", newline='', mode='r') as f:
    reader = csv.reader(f)
...
for j in range(len(new_command_list)):
    #" dir dir"は当該コマンドの直前に
    if(j+1 == len(new_command_list)):
        for i in range(len(Dockerfile_Command)):
            if (str(Dockerfile_COPY[j]) in str(Dockerfile_Command[i])):
                Dockerfile_Command.insert(i+1,new_command_list[j])
    #" . ."はWORKDIR直後に
    else:
        for i in range(len(Dockerfile_Command)):
            if ("WORKDIR" in str(Dockerfile_Command[i])):
                Dockerfile_Command.insert(i+1,new_command_list[j])
...
with open(fiile_name+".mem", mode='w', newline='') as f:
    f.writelines(new_command)
#実行日を記憶
Update_count[0][0]=Update_count[0][-1]
with open(fiile_name+".csv", mode='w', newline='') as f:
    writer = csv.writer(f)
